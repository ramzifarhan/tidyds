...
`table1` termasuk "Tidy data" karena variabel sudah sesuai pada baris dan kolom nya

```{r}
table2
```

`table2` tidak termasuk "Tidy data" karena dua data ditumpuk jadi satu, cases dan population

```{r}
table3
```

`table3` tidak termasuk "Tidy data" karena 2 informasi berbeda ada pada 1 kolom yang sama
...
`table4a` tidak termasuk "Tidy data" karena data tidak sesuai pada tempatnya, dan salah pada judul

```{r}
table4b
```

`table4b` tidak termasuk "Tidy data" karena data tidak berada pada tempat yang sesuai
```{r}
table5
```

`table5` tidak termasuk "Tidy data" karena ada kolom yang malah dipisah, jenis data pun salah, kolom rate pun salah
...
```
```{r}
table4a_tidy <- gather(table4a, key = "year", value = "cases", 2:3, convert = FALSE)
table4a_tidy
```
...
1. menentukan data yang akan dipakai
2. menyusun kolom-kolom utama sesuai negara dan tahun
3. mengelompokkan data sesuai kolom dari tabel tertentu
4. menjadikan data ganda menjadi satu data saja
5. mengeluarkan data dari tb4
6. menggabungkan data populasi dari tb5 dan populasi
7. memisahkan data populasi kedalam tb6
8. membuat kolom baru/memodifikasi kolom lama untuk menampilan persentase dari case/population

Berikan komentar Anda pada dua jenis cara penulisan kode di atas! Menurut Anda cara penulisan manakah yang lebih mudah dilakukan dan dipahami? menurut saya, lebih mudah untuk memahami penulisan yang pertama, namun dengan penulisan yang kedua, proses lebih mudah dilakukan karena tidak usah menuliskan key dan value nya lagi
