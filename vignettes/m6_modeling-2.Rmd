...
unnest tokens menghapus tanda baca yang ada.
stop words itu kata-kata yang tak umum
anti join, mencari kata, lalu jka ada index yg tidak sama, akan dihapus
...
15 baris pertama berdasarkan tf idf terbesar
...
```{r}
sherlock_gamma %>% 
  ggplot(aes(x = rev(story), y = gamma, fill = as.character(topic))) +
  geom_col() +
  coord_flip()
```
