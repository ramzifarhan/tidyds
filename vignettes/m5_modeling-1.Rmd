...
un_smp %>% 
  ...
  facet_wrap(~tahun) +
  theme_light()
fit2 <- aov(ipa ~ tahun + status + tahun:status, data = un_smp)
anova(fit2)
...
```
bahasa inggris sudah menghilang  di tahap training dan testing karena dianggap korelasinya tinggi dengan yang lain sehingga tidak dibutuhkan lagi 
...
nama sekolah tidak diikutkan karena termasuk id
ipa~. artinya = ipa prediksi dengan semuanya
...
rsq (rsquare) skala 0-1
rmse dan mae selisih nilai prediksi dengan nilai real
rmse sensitif terhadap error yang besar
mae lebih stabil
...
```
```{r}
save(un_smp_recipe, file = here("data", "un_smp_recipe.rda"),
     compress = "bzip2", compression_level = 9)
save(un_smp_lm, file = here("data", "un_smp_lm.rda"), compress = 
       "bzip2", compression_level = 9)
save(un_smp_rf, file = here("data", "un_smp_rf.rda"), compress = 
       "bzip2", compression_level = 9)
```
